...
- `VideoGalleryScreen` は端末内動画だけを抽出し、フォルダ単位のカテゴリ一覧として表示する。
- フォルダ選択後は `GalleryGridView` で動画一覧を表示し、タップした動画を画面内の `VideoMiniPlayer` で即時プレビュー再生する。
- ミニプレイヤーの再生対象は、グリッドがクリック時に返す実際の `MediaData` リストと URI から決定し、表示順や内部フィルタによる index ずれを防ぐ。
- 全画面再生は `VideoFullscreenViewerScreen` に同じ動画リストと開始 index を渡し、プレビューと全画面で同じ動画を開く。
- 動画画面のシステムバー領域と背景はアプリテーマの `topBar` / `background` 色に合わせ、白い余白を出さない。
