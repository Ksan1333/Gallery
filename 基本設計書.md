...
| 参照画像 | URL / ローカルパス | 参照プロジェクト管理、ローカル保存 |
...
| 参照プロジェクト | `references` | 参照画像プロジェクトの一覧、詳細、検索追加を扱う。 |
...
- `ReferenceRepository` が Room と Documents/Gallery/References 配下のファイルを管理する。
- URL から参照画像をダウンロードし、`reference_items.localUri` に保存先を記録する。
- プロジェクトは `ACTIVE` / `FINISHED` ステータスを持つ。
- プロジェクト削除時はローカル参照フォルダも削除し、Room は CASCADE でアイテムを削除する。
