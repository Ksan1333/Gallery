...
| 背景 | BLACK / GRAY / WHITE |
| 描画品質 | STANDARD / HIGH |
| 画面割り当て | 3 / 4 / 5 / 7 / 11 分割。中心領域と周辺領域を組み合わせ、前後ページ、前後本、メニュー、ズーム、設定、スライドショーなどを割り当てる。 |
...
- タッチインジケーターが有効な場合、現在の画面割り当てを中心領域と周辺領域の枠で表示し、各領域の操作名を重ねて表示する。
- 長押し拡大鏡は、長押ししている間だけ倍率を上げ、指を動かすと拡大位置が追従する。指を離したら長押し前の倍率と位置に戻す。
- 画面回転後もシステムバー制御を再適用し、コントロール表示中のボタンがOSナビゲーションバーに隠れないようにする。

### 4.4. 機能内画面遷移図

漫画一覧、漫画ビューア、その他メニュー、表示設定ボトムシート、しおり一覧、しおりジャンプの流れを、画面タイトル付きのミニUIモックと矢印で示す。

![漫画ビューア 機能内画面遷移図](../images/detail_design/06_book_viewer_transition.svg)
