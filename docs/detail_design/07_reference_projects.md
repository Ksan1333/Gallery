...
| 番号 | UI部品 | 機能 |
| --- | --- | --- |
| 1 | メニュー | 資料プロジェクト画面から他機能へ移動する。 |
| 2 | プロジェクトカード | 制作単位の資料セットを開く。進行中/完了状態を表示する。 |
| 3 | 新規作成FAB | 新しいお絵描き資料プロジェクトを作成する。 |
| 4 | 検索バー/カメラ | WebViewで資料検索し、検索画面スクショを保存する。 |
| 5 | 資料グリッド | 資料を2列表示し、全画面参照、保存、削除を行う。 |
