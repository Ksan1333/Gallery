...
| 番号 | UI部品 | 機能 |
| --- | --- | --- |
| 1 | メニュー | ダウンローダーから他画面へ移動する。 |
| 2 | URL入力欄 | 共有URL、クリップボードURL、手入力URLを表示・編集する。 |
| 3 | オプション表示ボタン | URLを解析し、保存候補と品質選択を表示する。 |
| 4 | ダウンロード履歴 | 保存済みメディア、状態、日時、プレビュー導線を表示する。 |
| 5 | 品質選択ダイアログ | High/Medium/Low、GIF保存、開始、HOME、キャンセルを選ぶ。 |
