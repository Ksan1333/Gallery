...
| 番号 | UI部品 | 機能 |
| --- | --- | --- |
| 1 | メニュー | ナビゲーションドロワーを開き、主要画面へ移動する。 |
| 2 | フィルタ/ソート/列数 | メディア種別、年齢制限、端末向け比率、並び順、表示密度を切り替える。 |
| 3 | グルーピング見出し | 日/月/年/ストレージ単位の区切りを表示する。 |
| 4 | メディアセル | タップでビューア、長押しで選択モードに入る。 |
| 5 | 選択モードバー | お気に入り、一括タグ・評価編集、フォルダ移動、削除を実行する。 |
| 6 | ボトムナビ | ホーム、フォルダ、My List、漫画、ゴミ箱などへ移動する。 |
