...
![おすすめ・視聴履歴UIモック](../images/detail_design/08_recommendations_history_ui.svg)

#### 視聴履歴リスト

![視聴履歴リストUIモック](../images/detail_design/08_viewing_history_ui.svg)
...
| 番号 | UI部品 | 機能 |
| --- | --- | --- |
| 1 | おすすめ空状態 | 閲覧データがない場合、計測モードでの閲覧を促す案内を中央表示する。 |
| 2 | 視聴履歴リスト | 最近見たメディア、閲覧回数、閲覧時間、再表示導線を表示する。 |
| 3 | 関連パネル | ビューア内で現在メディアに近い候補をスコア付きサムネイルで表示する。 |

### 4.4. 機能内画面遷移図

おすすめ、視聴履歴、ビューア、関連パネル、候補ビューア、閲覧統計更新の関係を、画面タイトル付きのミニUIモックと矢印で示す。

![おすすめ・視聴履歴 機能内画面遷移図](../images/detail_design/08_recommendations_history_transition.svg)
