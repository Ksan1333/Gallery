...
基本設計書には全体ユースケース、全体利用フロー、ER 図、主要シーケンス、API・外部連携、保存先、制約をまとめています。各詳細設計には機能別の画面説明、UIモック、機能内画面遷移図、ユースケース図、操作フロー、関連 DB、DAO/Repository、シーケンス、利用 API を記載しています。画面遷移図の索引から、各機能の遷移図へ移動できます。リグレッション試験書には、アプリ動作を横断確認する試験観点と実施結果を書き込めるTSVシートを用意しています。
