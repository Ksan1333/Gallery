...
基本設計書には全体ユースケース、全体利用フロー、ER 図、主要シーケンス、API・外部連携、保存先、制約をまとめています。画面遷移図には、主要画面、画面内ビューア、ダイアログ、ポップアップ、ボトムシートの遷移をUIモック付きでまとめています。各詳細設計には機能別の画面説明、UIモック、ユースケース図、操作フロー、関連 DB、DAO/Repository、シーケンス、利用 API を記載しています。リグレッション試験書には、アプリ動作を横断確認する試験観点と実施結果を書き込めるTSVシートを用意しています。

- [更新履歴 / CHANGELOG](app/src/main/assets/CHANGELOG.md)
- [リグレッション試験書](docs/test/regression_test_plan.md)
- [リグレッション試験実施シート TSV](docs/test/regression_test_cases.tsv)
- [基本設計書](基本設計書.md)
- [画面遷移図](docs/画面遷移図.md)
...
このリポジトリでは、上記の Kotlin コンパイルとローカル単体テストを基本の確認コマンドとして使います。
